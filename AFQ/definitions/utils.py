...
class Definition(object):
    ...
    def str_for_toml(self):
        ...
        return (
            type(self).__name__
            + "("
            + _arglist_to_string(self.__init__.__code__.co_varnames, get_attr=self)
            + ")"
        )
...
def find_file(
    ...
    path_subject = bids_layout.parse_file_entities(path).get("subject", None)
    file_subject = bids_layout.parse_file_entities(nearest).get("subject", None)
    path_session = bids_layout.parse_file_entities(path).get("session", None)
    file_session = bids_layout.parse_file_entities(nearest).get("session", None)
