...
streamlines = [
    np.array([[1, 1, 1], [2, 1, 1], [3, 1, 1], [4, 1, 1]]),
    np.array([[1, 1, 2], [2, 1, 2], [3, 1, 2], [4, 1, 2]]),
...
def test_check_sls_with_inclusion():
    ...
    result = list(check_sls_with_inclusion(sls, include_rois, include_roi_tols))
...
def test_check_sl_with_exclusion_pass():
    result = check_sl_with_exclusion(streamline1, exclude_rois, exclude_roi_tols)
    assert result is False


def test_check_sl_with_exclusion_fail():
    result = check_sl_with_exclusion(streamline2, exclude_rois, exclude_roi_tols)
    assert result is True
