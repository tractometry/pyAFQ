...
bundles = {
    "Left Corticospinal": {
        "include": [templates["CST_roi1_L"], templates["CST_roi2_L"]],
        "prob_map": templates["CST_L_prob_map"],
        "cross_midline": None,
    },
    "Right Corticospinal": {
        "include": [templates["CST_roi1_R"], templates["CST_roi2_R"]],
        "prob_map": templates["CST_R_prob_map"],
        "curvature": {"sft": cst_r_curve_ref, "cut": True, "thresh": 45},
        "cross_midline": None,
    },
}
...
def test_segment_return_idx():
    ...
    for idx, sl in enumerate(fiber_groups["Right Corticospinal"]["sl"].streamlines):
        idx_sl = tg.streamlines[fiber_groups["Right Corticospinal"]["idx"]][idx]
...
def test_exclusion_ROI():
    ...
    slf_tg = StatefulTractogram(
        np.asarray(
            [
                [[8, 53, 39], [8, 50, 39], [8, 45, 39], [30, 41, 61], [28, 61, 38]],
...
def test_segment_sampled_streamlines():
    ...
    npt.assert_(
        0
        <= len(sampled_fiber_groups["Right Corticospinal"])
        <= len(fiber_groups["Right Corticospinal"])
    )
