...
def test_str_instantiates_mixin():
    ...
    npt.assert_(thresh_image.lower_bound == thresh_image_from_str.lower_bound)
    npt.assert_(thresh_image.upper_bound == thresh_image_from_str.upper_bound)
...
def test_resample_image():
    ...
    npt.assert_array_equal(resampled_data, image_data)
    ...
    npt.assert_array_equal(resampled_image.shape, dwi_data[..., 0].shape)
    npt.assert_equal(resampled_image.dtype, image_data.dtype)
    assert is_resampled
