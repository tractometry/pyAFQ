# Original source: github.com/dipy/dipy
# Copyright (c) 2008-2025, dipy developers.
# Licensed under the 3-clause BSD license
# Modified by John Kruper for numba MSMT testing
# Will ultimately be moved upstream to DIPY
...
def test_mcsd_model_delta():
    ...
    with warnings.catch_warnings():
        ...
        response = multi_shell_fiber_response(
            sh_order_max, [0, 1000, 2000, 3500], wm_response, gm_response, csf_response
        )
    with warnings.catch_warnings():
        warnings.filterwarnings(
            "ignore",
...
def test_MultiShellDeconvModel_response():
    ...
    npt.assert_raises(
        ValueError, MultiShellDeconvModel, gtab, np.ones((3, 3, 4)), iso=3
    )
...
def test_multi_shell_fiber_response():
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        response = multi_shell_fiber_response(
            sh_order_max, [1000, 2000, 3500], wm_response, gm_response, csf_response
        )
...
def test_mask_for_response_msmt(rng):
    ...
    with warnings.catch_warnings(record=True) as w:
        wm_mask, gm_mask, csf_mask = mask_for_response_msmt(
            gtab,
            data,
            roi_center=None,
            roi_radii=(1, 1, 0),
...
def test_response_from_mask_msmt(rng):
    ...
    response_wm, response_gm, response_csf = response_from_mask_msmt(
        gtab, data, masks_gt[0], masks_gt[1], masks_gt[2], tol=20
    )
...
def test_auto_response_msmt(rng):
    ...
    with warnings.catch_warnings(record=True) as w:
        response_auto_wm, response_auto_gm, response_auto_csf = auto_response_msmt(
            gtab,
            data,
            tol=20,
            roi_center=None,
        ...
        response_from_mask_wm, response_from_mask_gm, response_from_mask_csf = (
            response_from_mask_msmt(gtab, data, mask_wm, mask_gm, mask_csf, tol=20)
        )
