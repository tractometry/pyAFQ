...
def test_fit_dki_inputs():
    ...
    pytest.raises(ValueError, dki.fit_dki, data_files, bval_files, bvec_files)


def test_fit_dki():
    fdata, fbval, fbvec = dpd.get_fnames("small_101D")
    with nbtmp.InTemporaryDirectory() as tmpdir:
        file_dict = dki.fit_dki(str(fdata), str(fbval), str(fbvec), out_dir=tmpdir)
...
def test_predict_dki():
    with nbtmp.InTemporaryDirectory() as tmpdir:
        ...
        predict_fname = dki.predict(params_file, gtab, S0_file=fdata, out_dir=tmpdir)
