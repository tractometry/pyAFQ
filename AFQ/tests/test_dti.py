...
def test_noise_from_b0():
    ...
    with nbtmp.InTemporaryDirectory() as tmpdir:
        ...
        img, data, gtab, mask = ut.prepare_data(
            fdata, fbval, fbvec, mask=mask, b0_threshold=50
        )
...
def test_fit_dti():
    ...
    with nbtmp.InTemporaryDirectory() as tmpdir:
        file_dict = dti.fit_dti(
            [fdata1, fdata2], [fbval1, fbval2], [fbvec1, fbvec2], out_dir=tmpdir
        )
...
def test_predict_dti():
    with nbtmp.InTemporaryDirectory() as tmpdir:
        ...
        predict_fname = dti.predict(params_file, gtab, S0_file=fdata, out_dir=tmpdir)
        ...
        predict_fname = dti.predict(params_file, gtab, S0_file=fdata, out_dir=tmpdir)
        prediction = nib.load(predict_fname).get_fdata()
        npt.assert_almost_equal(prediction[mask], nib.load(fdata).get_fdata()[mask])
