...
def pve_internal(structural_imap, pve="synthseg"):
    ...
    pve : str or PVEImage, optional
        ...
        reccomended if they are available.
    ...
    raise ValueError(
        "pve must be a PVEImage, PVEImages, "
        "'synthseg', or 'multiaxial+brainchop'")
...
@as_file(suffix='_model-msmtcsd_param-fod_dwimap.nii.gz',
         subfolder="models")
@as_img
def msmt_params(data_imap, pve_internal,
                msmt_sh_order=8,
                msmt_fa_thr=0.7):
    """
    ...
    csf = resample(pve_data[..., 0], data_imap["data"][..., 0],
                   pve_img.affine, data_imap["dwi_affine"]).get_fdata()
    gm = resample(pve_data[..., 1], data_imap["data"][..., 0],
                  pve_img.affine, data_imap["dwi_affine"]).get_fdata()
    wm = resample(pve_data[..., 2], data_imap["data"][..., 0],
                  pve_img.affine, data_imap["dwi_affine"]).get_fdata()
    ...
    mcsd_fit = mcsd_model.fit(
        data_imap["data"], mask, n_cpus=data_imap["n_cpus"])
...
@as_file(suffix='_model-msmtcsd_param-nufid_dwimap.nii.gz',
         subfolder="models")
@as_img
def msmt_aodf_nufid(msmt_aodf_params, data_imap,
                    pve_internal):
    """
    full path to a nifti file containing
...
def get_tissue_plan(kwargs):
    ...
    if isinstance(pve, PVEImages):
        ...
        tissue_tasks["csf_res"] = immlib.calc("pve_csf")(as_file(
            '_desc-csf_probseg.nii.gz', subfolder="models")(
                pve.probseg_funcs[0]))
        tissue_tasks["gm_res"] = immlib.calc("pve_gm")(as_file(
            '_desc-gm_probseg.nii.gz', subfolder="models")(
                pve.probseg_funcs[1]))
        tissue_tasks["wm_res"] = immlib.calc("pve_wm")(as_file(
            '_desc-wm_probseg.nii.gz', subfolder="models")(
                pve.probseg_funcs[2]))
