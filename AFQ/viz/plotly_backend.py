...
def _draw_streamlines(
    ...
    figure.add_trace(
        go.Scatter3d(
            ...
            marker=dict(
                size=0.0001, color=_color_arr2str(np.maximum(color - 0.2, 0))
    ...
    figure.update_layout(legend=dict(font=dict(color="white")))
...
def _plot_profiles(profiles, bundle_name, color, fig, scalar):
    ...
    fig.update_layout(
        ...
        scene2=dict(
            ...
            dragmode=False,
            xaxis=dict(
                title=dict(text="Location", font=font), tickfont=dict(color="white")
            ),
            yaxis=dict(
...
def visualize_bundles(
    ...
    **kwargs,
    ...
    include_profiles : Tuple of Pandas Dataframe and string
        ...
        output format for the bundle(s) being displaid. It will be used
